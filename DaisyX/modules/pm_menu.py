...
STICKERS = (
    "CAACAgIAAxkBAAEKAtdgqdnnkBYtB-v3TLocdt20Lf3ZAAN8FAAC6NbiEmW2QNC4eVGEHwQ",
)
...
async def get_start_func(message, strings, edit=False):
    ...
    buttons.add(
        InlineKeyboardButton(strings["btn_channel"], url="https://katarina.se.ke"),
        InlineKeyboardButton(
            strings["btn_group"], url="https://t.me/katarina_ox"
        ),
    )
    buttons.add(
        InlineKeyboardButton(
            "👸🏼 Add me to your group",
            url=f"https://telegram.me/Heroine_SnowBot?startgroup=true",
        )
    )
...
async def help_cmd_g(message, strings):
    ...
    button = InlineKeyboardMarkup().add(
        InlineKeyboardButton(text=text, url="https://t.me/Heroine_SnowBot?start")
    )
    await message.reply(strings["help_header"], reply_markup=button)
